...
Snapshot as of 2026-09-10. Update whenever a domain's write source changes.
...
| PRV-0.5 migration marker | `dune_state_v4.data.meta.recordsMigration` (Gen-2, inside the coordinated wrapper, schema 14) | — | **PRV-0.5 R2 + R4 + R5 + R6 (ADR-015 addenda #1, #3, #4, #5).** Migration authority lives INSIDE the same coordinated wrapper as the records, not as an out-of-band sticky flag. Canonical marker schema (R5): `{ status: 'migrated' \| 'unmigrated', schemaVersion: SCHEMA_VERSION, ...for unmigrated: priorSchemaVersion: <0..SCHEMA_VERSION-1>, reason: 'migrateUp-from-v<priorSchemaVersion>' }`. `defaultState()` (fresh browser + `Store.reset()`) initializes `{ status: 'migrated', schemaVersion: SCHEMA_VERSION, reason: 'default-state' }` with empty records — Reset cannot rehydrate legacy personal records. A pre-v14 wrapper migrated up marks `status: 'unmigrated'` with canonical provenance; app.js hydration flips it to `migrated` ONLY after durable persistence is re-read and verified. PRV-0.5 R5 (ADR-015 addendum #4): every authority consumer — hydration fast-path, `processImport`, snapshot restore, backup export, boot recovery — routes through the ONE Store-owned evaluator `Store.evaluatePersistedAuthority()` / `Store.evaluateCandidateWrapper()` / `Store.evaluateCandidateData()`. Six classifications: **AUTHORITATIVE_MIGRATED** (fast-path skip, backup accepted), **VERIFIED_LEGACY_TRANSITION** (the ONLY class that authorises `LEGACY_RECORDS` seeding — requires an OBSERVED outer legacy wrapper via the transaction-scoped `Store.canAuthoriseLegacySeed()` capability; marker text alone is NOT proof — R6 P1-1), **MALFORMED_CURRENT_SCHEMA** (recovery required; hydration does NOT synthesize `[]` and does NOT seed legacy), **CORRUPT_STALE_COLLIDING** (invalid/stale/collision/blocker — recovery required; backup refused), **UNSUPPORTED_FUTURE_SCHEMA** (`version > SCHEMA_VERSION` rejected at every boundary), **ABSENT**. Boot into corrupt disk sets a `STORE_CORRUPT_AUTHORITATIVE_STATE` durability blocker AND preserves the corrupt bytes as evidence; normal backup export refuses invalid authority (quarantine path via `window.exportRecoveryEvidence`). |
...
| Sync — Gist ID | `dune_gist_id_v1` (Gen-1) | No | Connected Gist for `saveConnected`/`loadConnected`; retargeted only via explicit `bootstrapOrReconnect`. New acknowledgements use a JSON string with byte-exact readback; readers also accept the prior raw-string representation. |
| Sync — accepted base | `dune_gist_sync_base_v1` (Gen-1) | No | Strict schema-1 JSON: `{ schema, gistId, remoteVersion, baseDataHash, acceptedAt }`. The semantic SHA-256 excludes Store transaction metadata regenerated by import. Every write is byte-readback/reparse verified; malformed, unowned, or failed-write records are non-authoritative and invalidated. |
| Sync — prior remote audit | `dune_gist_prior_remote_revision_v1` (Gen-1) | No | Exact remote ID/revision/hash recorded and byte-readback verified before **Keep Local** or bootstrap **Use Local** may overwrite remote data. Failure is a hard zero-PATCH gate. |
| Sync — preserved local conflict export | `dune_gist_conflict_export_v1` (Gen-1) | No | Byte-readback-verified local backup plus conflict context written before a destructive conflict/bootstrap Load Remote can proceed. |
