...
### ADR-015 addendum #15 (2026-09-09) — Round-7 remediation for Codex Round-6 exact-SHA review

**Context.** Codex's independent HIGH-risk exact-SHA review of `68f53c4d697f99e2feaf1dd18552aadf39da4a2f` (Round-6 Pre-Push Amendment closure) returned `FAIL — NOT SAFE — MERGE BLOCKED` with two binding P1 findings, three P2 findings, and one P3 finding. This addendum records the Claude-authored narrow local remediation on top of `68f53c4d…` on branch `claude/prv-0-5-round7-recovery-adoption-remediation`; nothing has been pushed. The Codex-authored `ef6e4019beb7f5624d40c4c80d2b0c41a6029481` commit is NOT an ancestor.

**P1-01 — recovery revision exhaustion must fail before mutation.** `commitFullStateWrapper` previously guarded exhaustion with `if (diskRevision >= Number.MAX_SAFE_INTEGER)` — durable disk revision alone. Because `nextRevision` was then computed as `max(diskRevision, knownRevision, _transitionAuth.knownRevisionAtIssue) + 1`, an accepted baseline of `Number.MAX_SAFE_INTEGER` (in-memory or on the recovery auth) combined with a regressed disk yielded `9007199254740992` — not a safe integer, therefore not a legal authoritative revision. Fix (`core.js`): compute `_exhaustionBaseline = max(diskRevision, knownRevision, _transitionAuth.knownRevisionAtIssue)` BEFORE quarantine allocation and BEFORE any primary write; if `_exhaustionBaseline >= Number.MAX_SAFE_INTEGER`, return `{ok:false, error:'STORE_REVISION_EXHAUSTED', baseline, diskRevision, knownRevision, authRevisionAtIssue}`. The same variable is then reused where `nextRevision` is minted so the two computations cannot drift. Single guard covers Reset, Snapshot Restore, processImport-recovery, and atomic legacy conversion (all callers share `commitFullStateWrapper`). Behavioural claims: primary bytes byte-exact preserved, no quarantine allocated, no snapshot written, no success publication, no subscriber notify. Tests: `R7-CODEX-R6-P1-01a-RESET-EXHAUSTION-KNOWN-REVISION-MAX`, `R7-CODEX-R6-P1-01b-SNAPSHOT-RESTORE-EXHAUSTION`, `R7-CODEX-R6-P1-01c-BOUNDARY-MAX-MINUS-ONE-ADVANCES-ONE-STEP`.

**P1-02 — external authority adoption at settlement must update the public Store.** `endFullStateTransaction`'s newer-external-wrapper adoption branch previously advanced `baseState`/`knownRevision`/`committedAt`/`baseWrapperRaw` but never called `rebuildOptimistic()`/`notifyAll()` on the no-commit path — `Store.get()` stayed stale, subscribers never observed the adopted authority. The settlement return still said `settlement:'FULL_STATE_NOT_COMMITTED'`, indistinguishable from "nothing happened", so listeners could not react to convergence. Fix (`core.js`): declare `externalAdopted = null` at the top of settlement; the adoption branch records `{priorRevision, adoptedRevision, adoptedCommittedAt}` into it before mutating internal state. The completion block: `externalAdoptionThisTx = !committedThisTx && externalAdopted !== null`. On external adoption OR local commit, run `rebuildOptimistic() + notifyAll()`. On external adoption, additionally emit a distinct `lifeos:store-external-adoption` event with the transition details. Settlement returns `settlement:'FULL_STATE_EXTERNAL_ADOPTION'` (never `FULL_STATE_COMMITTED`) with an `externalAdoption` field carrying the same transition details; the `lifeos:store-freeze-end` event's `detail.externalAdoption` is `true` for this outcome. After settlement, durable primary / internal base / internal known revision / raw authoritative wrapper / `Store.get()` / public optimistic state all agree on the adopted generation. No stale deferred storage event is required later. Test: `R7-CODEX-R6-P1-02a-TWO-PAGE-EXTERNAL-ADOPTION-PUBLISHES-STORE` (two pages in one Playwright context).

**P2-01 — canonical architecture documentation.** `docs/lifeos/ARCHITECTURE.md`'s Authority-evaluator section previously described `_legacyTransitionCapability` and support for all `0..SCHEMA_VERSION-1`. Fix: the VERIFIED_LEGACY_TRANSITION row now names the exact-source `_transitionAuth` model (`sourceRawBytes` byte-match under the destructive lock) and the frozen v8..v13 supported historical range; v0..v7 fail closed via `validateLegacySourceRequiredFields`; the current v14 is strict canonical; and a schema-14 wrapper's marker text ALONE is not proof of an outer transition. The auth `_consumeTransitionAuth()` fires on adoption of different bytes. No production behavior change.

**P2-02 — recovery matrix and revision claims match runtime.** Two docs updates in `docs/lifeos/ARCHITECTURE.md`. (a) The revision-exhaustion bullet now names the complete accepted monotonic baseline (`max(durableDiskRevision, knownRevision, transitionAuth.knownRevisionAtIssue)`), specifies the single guard shared by every full-state recovery family, and explicitly lists the "byte-exact preserved / no quarantine / no snapshot / no publication" outcome. (b) A new "Recovery / durability blocker matrix (runtime — describes actual behavior)" section enumerates the runtime outcomes as ten rows: corrupt disk under recovery, recovery source-generation change under lock, post-write byte-verify failure, post-write authority-classification failure, revision exhaustion (NEW), ordinary CAS silent-no-op, external clear of `STATE_KEY`, external-authority adoption at settlement (NEW), no-lock refusal, legacy-conversion source-identity mismatch. Each row names the blocker (or "unchanged"), primary-bytes fate, quarantine fate, public-Store rebuild fate, and the publication event that fires (with the correct distinction between success, failure, and convergence).

**P2-03 — test-only fault switches gated from production runtime.** `Store._testForceNoLock(bool)` and `Store._testForcePostWriteEvalFailure(bool)` were publicly available on production `window.Store`, so an XSS payload / rogue extension could invoke them and disable the destructive Web Lock check or drive post-write eval failure. Fix (`core.js`): the two setters are attached to the returned Store object ONLY when Store construction observes `window.__LIFEOS_TEST_ENV__ === true`. Production runtime never sets the marker, so both hooks are `undefined` on `window.Store` in production. Playwright specs that exercise the fault branches set the marker via `page.addInitScript` BEFORE navigation; three existing sites in `tests/prv-preservation.spec.js` (FINAL-L1 / FINAL-L2 / R4-P1-01d) were updated with a two-line marker step. Tests: `R7-CODEX-R6-P2-03a-PRODUCTION-HOOKS-UNEXPOSED` (marker not set → both hooks are `typeof undefined` and NOT own-properties of `window.Store`), `R7-CODEX-R6-P2-03b-TEST-ENV-HOOKS-EXPOSED` (marker set → both hooks are functions, invocation does not throw). No new build system, no new framework — uses the existing `page.addInitScript` mechanism.

**P3-01 — durable historical value-preservation oracle.** `tests/prv-value-preservation-oracle.spec.js` (new file, deliberately independent of `R6A_EVIDENCE` and any Store code). Defines an inline literal `SENTINELS` table with non-default values for every field the Codex brief enumerates: BHT (`habits`, `entries`, `snapshots`, `lifeEvents`, `vocab.triggers`, `vocab.coping`, `vocab.moods`, `ai.provider`, `ai.ollamaUrl`, `ai.model`, `meta`), Telemetry (`accumulatedFatigue`, `weeklyShiftHours`, `focusReserve`), Money (`salary_net`, `expenses`). For each supported historical version v8..v13, the test builds a valid source wrapper, runs `Store.evaluateCandidateWrapper(wrapper)`, and asserts every sentinel survives byte-exact in the migrated candidate. ADR-005 exception is asserted separately: `bht.ai.apiKey` MUST NOT exist as an own-property in the migrated `bht.ai` — the deliberately-narrowed provenance surface never widens. `bht.meta.__oracle_marker__` / `__oracle_note__` prove `bht.meta` preservation without asserting on migration-managed fields (`version` is correctly rewritten to the migrated schemaVersion). `bht.ai.provider` uses `'ollama'` because sanitizeAI's post-migration normalization is the only accepted non-default; using the accepted-non-default value proves BOTH that migration carried the raw value through AND that later sanitization would not re-erase it. Because the sentinel table lives in the spec, not in `R6A_EVIDENCE`, a production matrix corruption bug that quietly rewrites expected preservation cannot rewrite this oracle.

**Preservation matrix (nothing regressed).** BINDING-1 (no lock = fail closed) — unchanged from addendum #14. BINDING-2 (one durable-write contract; post-write uncertainty) — unchanged from addendum #14. BINDING-3-A (frozen Historical-Version Matrix, v8..v13) — unchanged. BINDING-3-B (frozen Blocker/Recovery Matrix) — EXTENDED with two rows: revision-exhaustion (P1-01) and external-authority-adoption at settlement (P1-02); each row is testable and tested. All previously closed R7 P1/P2 findings (current v14 validation before normalization, omitted/null/malformed/array Logbook rejection, true-ABSENT-only fresh defaults, complete BHT/telemetry validation, v8..v13 evidence-backed historical validation, version-specific Logbook representation, v0..v7 fail-closed, future/versionless fail-closed, exact-source atomic legacy conversion, no-lock fail-closed, post-write uncertainty five fault families, no-success-effects under post-write uncertainty, quarantine retention through uncertainty, v14 Snapshot Restore revision validation, recovery UX reachability/confirmation/cancellation, ordinary CAS durable-reread) still pass cold.

**Cold test evidence.**

- Full Playwright suite `--retries=0`, single worker, port 4173 cleared before run: **360 / 360 pass in 3 min 20 sec.**
- `npx playwright test --list` → **Total: 360 tests in 10 files.**
- `npx playwright test tests/prv-preservation.spec.js --list` → **Total: 197 tests in 1 file.**
- `npx playwright test tests/store-durability.spec.js --list` → **Total: 48 tests in 1 file.**
- `npx playwright test tests/prv-value-preservation-oracle.spec.js --list` → **Total: 6 tests in 1 file.**
- `node --check` on `core.js`, both edited spec files, and the new oracle spec: clean.
- `git diff --check` against `68f53c4d…`: clean.
- Working tree: clean after commit.
- No tracked test was removed, weakened, skipped, or rewritten materially.

**Provenance.** Round-7 remediation ancestry: `<Round-7 head> → 68f53c4d697f99e2feaf1dd18552aadf39da4a2f`. The Codex-authored `ef6e4019beb7f5624d40c4c80d2b0c41a6029481` is NOT an ancestor. Worktree `/Users/mahmoudmarwan/LIFE/.claude/worktrees/prv-0-5-round7-recovery-adoption`. Branch `claude/prv-0-5-round7-recovery-adoption-remediation` (local only, unpushed).

**Date:** 2026-09-09 (Round-7 remediation on top of Codex Round-6 review of `68f53c4d…`; local only, unpushed pending ChatGPT pre-push review then Codex final independent HIGH-risk re-review).

### ADR-015 addendum #16 (2026-09-09) — Round-8 remediation for Codex Round-7 exact-SHA re-review

**Context.** Codex's independent HIGH-risk exact-SHA re-review of `351236dc4ae65168089db8f4169b5a2a193eab69` (Round-7) returned `FAIL — NOT SAFE — MERGE BLOCKED` with two binding P1 findings, three P2 findings, and one P3 finding. This addendum records the Claude-authored narrow local remediation on top of `351236dc…` on branch `claude/prv-0-5-round8-external-authority-recovery`; nothing has been pushed. The Codex-authored `ef6e4019…` is NOT an ancestor.

**P1-01 — ONE strict external-authority admission boundary.** Codex reproduced that storage-event adoption and full-state settlement bypassed the strict candidate evaluator: `commitLocked`, `endFullStateTransaction`, and `onStorage` all used `migrateAndValidate(parsed)` — a permissive path that runs `migrateUp` (default-fills missing BHT/telemetry/history fields) then only checks `validate()` (which requires salary_net + qatarVisit). Result: a partial v13 wrapper missing required fields could be default-filled and adopted as apparently verified authority; a v7 wrapper (v0..v7 are unsupported per BINDING-3-A) could be adopted; either could subsequently qualify for normal backup export. Fix (`core.js`): new helper `_admitExternalWrapper(rawWrapper)` inlines the strict evaluator surface — `parseWrapperRaw` → version classification (v14 canonical / v8..v13 legacy / v>SCHEMA / anything else) → source-shape validation BEFORE `migrateUp` (`validateFullStateCanonical` for v14, `validateLegacySourceRequiredFields` for v8..v13) → `migrateUp` → `normalizeLogbookDomain` → `validate` → `evaluateCandidateData` — and returns either `{ok:true, parsed, data, classification}` (AUTHORITATIVE_MIGRATED or VERIFIED_LEGACY_TRANSITION only) or `{ok:false, blockerCode:'STORE_CORRUPT_AUTHORITATIVE_STATE', reason}`. `commitLocked` (external reread), `endFullStateTransaction` (settlement adoption — only when disk raw differs from baseline), and `onStorage` (both newer-revision and equal-revision reread branches) all route through this helper. `isSupportedLegacySourceVersion` was tightened from `v >= 0` to `v >= 8` so `classifyMarker` and `evaluatePersistedAuthority` also refuse unsupported legacy versions consistently. Tests: `R8-CODEX-R7-P1-01a`..`R8-CODEX-R7-P1-01e` cover partial v13, v7, v99 future, versionless, and cross-path classification consistency.

**P1-02 — Source-bound blocker truth after cross-tab recovery.** Codex reproduced: Page A boots on corrupt bytes and installs `STORE_CORRUPT_AUTHORITATIVE_STATE` bound to those bytes; Page B does a Reset that installs a valid canonical authority; Page A adopts via storage-event; Page A's public state converges but the stale corrupt blocker survives — ordinary writes remain falsely blocked. Fix (`core.js`): `adoptExternal` (storage-event path) and the settlement adoption branch in `endFullStateTransaction` (transaction path) both clear source-bound blockers WHEN the newly adopted authority classifies as `AUTHORITATIVE_MIGRATED`. Cleared source-bound codes: `STORE_CORRUPT_AUTHORITATIVE_STATE`, `STORE_STATE_CLEARED_EXTERNAL`, `STORE_REVISION_REGRESSION`, `STORE_LEGACY_CONVERSION_PENDING`. Retained (not source-bound): `STORE_FULL_STATE_POST_WRITE_UNCERTAIN`, `STORE_REVISION_EXHAUSTED`, `STORE_ORDINARY_DURABLE_VERIFY_FAILED`, `STORE_READ_FAILED` — these describe this tab's own uncertain state, not a source generation the adoption proves away. `VERIFIED_LEGACY_TRANSITION` adoption does not clear blockers — a legacy source is a transition, not a settled resolution. A `lifeos:store-durability-cleared` event fires with `{reason:'external-adoption', priorBlocker}` so listeners can distinguish this from explicit `clearDurabilityBlocker()` calls. Tests: `R8-CODEX-R7-P1-02a` (corrupt A + Reset B clears blocker; ordinary write on A resumes), `R8-CODEX-R7-P1-02b` (malformed B candidate does NOT clear the blocker).

**P2-01 — Equal-revision divergent settlement collision.** `endFullStateTransaction`'s adoption branch previously accepted an equal-revision but divergent-raw wrapper silently. Fix (`core.js`): the adoption path detects `parsed.revision === knownRevision && rawNow !== baseWrapperRaw`, sets `externalAdopted.collision = true`, and emits `emitError({code:'STORE_REVISION_COLLISION', revision, where:'endFullStateTransaction'})` — matching the ordinary storage-event path (line 3477). Test: `R8-CODEX-R7-P2-01`.

**P2-02 — Local commit + newer external adoption at settlement.** Previously the settlement completion block used `externalAdoptionThisTx = !committedThisTx && externalAdopted !== null` — gating external-adoption reporting behind "no local commit." Codex reproduced: a local commit lands at revision N and a newer external wrapper lands at N+1 during freeze; settlement adopts external but returns only `settlement:'FULL_STATE_COMMITTED'`, falsely implying the local candidate is the final authority. Fix (`core.js`): `committedThisTx` and `externalAdoptionThisTx` are now tracked as INDEPENDENT booleans. Four legal outcomes: `FULL_STATE_COMMITTED`, `FULL_STATE_EXTERNAL_ADOPTION`, `FULL_STATE_COMMITTED_THEN_EXTERNAL_ADOPTION`, `FULL_STATE_NOT_COMMITTED`. Return payload now carries `commit` (bool) and `externalAdoption` (object with `afterLocalCommit` field) so consumers can react to either fact. The `lifeos:store-external-adoption` event's `detail.afterLocalCommit` reflects the same. Test: `R8-CODEX-R7-P2-02` proves the composition: local commit succeeds, Page B writes newer bytes, settlement returns `FULL_STATE_COMMITTED_THEN_EXTERNAL_ADOPTION`, public state reflects EXTERNAL authority (Page B's salary), not our committed candidate.

**P2-03 — Canonical docs match runtime truth.** `docs/lifeos/ARCHITECTURE.md` extended with three new bullet points and two updated ones (`Revision exhaustion` unchanged from Round-7; `External-authority admission` — new; `Settlement collision semantics` — new; `Local commit + external adoption composition` — new; `Source-bound blocker truth after cross-tab recovery` — new). All ADR-015 addenda are append-only; historical PASS statements in addenda #8–#15 remain untouched (they describe state at those addenda's landing dates). This addendum #16 is the truthful current state.

**P2-04 — Fault-hook threat wording.** `docs/lifeos/ARCHITECTURE.md` "Test-only fault hooks" paragraph rewritten. Previous wording ("XSS payload or rogue extension has no path") was too strong. Correct precision: normal production construction, where nothing on the page sets `window.__LIFEOS_TEST_ENV__` before `core.js` runs, does not expose the hooks on `window.Store`. Same-origin code that has already achieved script execution BEFORE `core.js` loads (a compromised extension with host-permissions running its own document-start script; an attacker who has already breached document-start) could set the marker just as legitimate Playwright tests do — that class of threat is outside the gate's guarantee and outside PRV-0.5's scope. ADR-005 + ADR-008 cover the broader same-origin model.

**P3 — Historical apiKey sanitization oracle.** Codex flagged that the R7 oracle asserted `bht.ai.apiKey` absence without seeding it — the assertion could not prove active stripping. Fix: (a) `tests/prv-value-preservation-oracle.spec.js` now seeds a literal sentinel `oracle-apiKey-sentinel-must-be-stripped-by-migration` on every v8..v13 source; assertions require both `hasOwnProperty('apiKey') === false` on the migrated `bht.ai` AND that the sentinel string is absent as any value under `bht.ai` (JSON substring check); (b) `core.js:migrateUp` now unconditionally strips `bht.ai.apiKey` at the end of every migration — the runtime BHT layer (`bht.js:sanitizeAI`) still runs at load, but the Store-level migration must strip too so external candidates arriving through `evaluateCandidateWrapper` / `_admitExternalWrapper` (import, snapshot, boot hydration, external adoption) never persist the key. Old backups written before ADR-005 landed with `bht.ai.apiKey` are sanitized. Oracle expectations are literal constants defined inline in the spec, not derived from `R6A_EVIDENCE` — production matrix corruption cannot rewrite them.

**Preservation matrix (nothing regressed).** All Round-7 invariants preserved: revision exhaustion baseline + guard ordering; MAX-1 → MAX exactly once; no quarantine/snapshot/write/notify on exhaustion; current v14 validation before normalization; Logbook omitted/null/malformed/array rejection; true-ABSENT-only defaults; complete current BHT/telemetry validation; strict v8-v13 import/snapshot/direct validation; future/versionless fail closed; exact-source atomic legacy conversion; no-lock fail closed; no authorization rebinding; durable reread/exact-byte/authority verification; post-write uncertainty zero-success effects; quarantine retention; ordinary CAS durable reread; snapshot revision validation; recovery UX confirmation/cancellation/keyboard behavior; production fault-hook absence; v8-v13 value preservation. BINDING-1/2/3-A/3-B all preserved. During remediation, five pre-existing tests initially failed after the strict admission change; they were fixed WITHOUT weakening any invariant: (a) `endFullStateTransaction` settlement only runs `_admitExternalWrapper` when disk raw differs from baseline (avoids overwriting a truthful `STORE_LEGACY_CONVERSION_PENDING` blocker with `STORE_CORRUPT_AUTHORITATIVE_STATE` when the disk hasn't moved); (b) auth invalidation via `_consumeTransitionAuth()` moved to fire on ANY disk-change in `onStorage` (before admission), so a storage-event with malformed bytes still invalidates a stale auth. These changes tightened the design, not weakened it.

**Cold test evidence.**

- Full Playwright suite `--retries=0`, single worker, port 4173 cleared before run: **370 / 370 pass in 3 min 27 sec.**
- `npx playwright test --list` → **Total: 370 tests in 10 files.**
- `npx playwright test tests/prv-preservation.spec.js --list` → **Total: 207 tests in 1 file.**
- `npx playwright test tests/store-durability.spec.js --list` → **Total: 48 tests in 1 file.**
- `npx playwright test tests/prv-value-preservation-oracle.spec.js --list` → **Total: 6 tests in 1 file.**
- Targeted `R8-CODEX-R7` suite (10 tests including two-page compositions): 10/10 pass in 13.5s.
- Oracle v8..v13 (6 tests): 6/6 pass in 2.9s.
- `node --check` on `core.js`, all edited spec files: clean.
- `git diff --check` against `351236dc…`: clean.
- Working tree: clean after commit.
- No tracked test was removed, weakened, skipped, or rewritten materially; three legacy tests received corrections that align them with the tightened admission rule while continuing to prove the SAME invariants.
