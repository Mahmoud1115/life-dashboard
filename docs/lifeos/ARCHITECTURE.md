...
- **External-authority admission** — storage-event adoption (`onStorage` → `adoptExternal`), full-state settlement adoption (`endFullStateTransaction`), and ordinary CAS pre-write external reread (`commitLocked`) all route through ONE strict `_admitExternalWrapper(rawWrapper)` helper (PRV-0.5 Codex Round-7 P1-01). The helper parses, classifies version, validates source-shape BEFORE `migrateUp` default-fill (`validateLegacySourceRequiredFields` for legacy v8..v13 sources; `validateFullStateCanonical` for v14 sources), then migrates and reclassifies inner authority. Only `AUTHORITATIVE_MIGRATED` and `VERIFIED_LEGACY_TRANSITION` are admissible; every other class installs `STORE_CORRUPT_AUTHORITATIVE_STATE` and internal state is not advanced. The supported historical range is v8..v13 (`isSupportedLegacySourceVersion`); v0..v7 fail closed. External adoption is only run when the disk raw actually differs from the Store's accepted baseline — a same-byte disk cannot silently overwrite an already-installed `STORE_LEGACY_CONVERSION_PENDING` blocker with a corruption blocker. Auth invalidation (`_consumeTransitionAuth`) runs the moment disk bytes change, even if the new bytes are subsequently rejected.
- **Settlement collision semantics** — an `endFullStateTransaction` adoption of an equal-revision but divergent-raw wrapper emits `STORE_REVISION_COLLISION` (same as the ordinary storage-event path), rather than silently converging (PRV-0.5 Codex Round-7 P2-01).
